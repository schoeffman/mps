...
const CREATE_PERFORMANCE_CYCLE = gql`
...
export function AddPerformanceCycleDialog() {
  ...
  const [createCycle, { loading }] = useMutation(CREATE_PERFORMANCE_CYCLE, {
  ...
  function handleOpenChange(nextOpen: boolean) {
    if (nextOpen) {
      ...
      setSelectedMonth("");
      setSelectedYear("");
      setUserIds(allUsers.map((u) => u.id));
    }
    setOpen(nextOpen);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    await createCycle({ variables: { input: { title, cycleMonth, userIds } } });
    setOpen(false);
  }
